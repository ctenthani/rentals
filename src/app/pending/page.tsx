...
type Submission = {
  ...
  tenants: {
    full_name: string;
    email: string | null;
    houses: { name: string; code: string } | { name: string; code: string }[];
  } | null;
};
...
export default function PendingPage() {
  ...
  async function load() {
    ...
    const { data, error: qErr } = await supabase
      ...
      .select(
        `id, amount, method, reference_used, paid_date, status, created_at, transaction_id, proof_url, notes,
  ...
  const confirm = async (id: string) => {
    ...
    const tenantInfo = row ? tenantOf(row) : null;
    if (tenantInfo?.email && data?.payment_id) {
              ...
              >
                <div className="flex justify-between gap-2">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">{info.house}</p>
                  </div>
                  <p className="font-bold text-emerald-700 text-lg">
                    {formatMK(Number(s.amount))}
                  </p>
                </div>
